...
`repo.toml` remains Agent-generated by convention (not tool-generated); the
...
`.vscode/settings.json`, or any future agent equivalent), that integration
is additive and must not be treated as a reason to remove/prune another
integration surface just because a different agent is running the current
session. The listed paths are examples, not an exhaustive list.
